...
An autonomous agent that searches YouTube for videos on a topic,
extracts transcripts, and generates comprehensive articles by
...
async def main():
    ...
    """
    log("=" * 60)
    log("YouTube to Article Writer Agent - Starting")
    log("=" * 60)
...
If a video doesn't have a transcript, skip it and move to the next one. Always try to use at least one video to generate an article."""
    ...
    try:
        log("Initializing agent with tools...")

        async with ClaudeSDKClient(options) as agent:
            log("Agent initialized successfully")

            # Start the autonomous workflow with a comprehensive query
            log("Starting autonomous workflow...")

            query = """Execute the complete article generation workflow with explicit data handling:
...
Begin execution now."""
            ...
            log("Processing agent responses...")
